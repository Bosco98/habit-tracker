...
A local-first, E2E-encrypted habit tracker with two habit types — personal and shared (mirrored across a circle). Accounts are keypairs with recovery phrases; sync rides encrypted through Jazz. Success = the daily check-in feels *physically satisfying*, partner data is always fresh, and the social loop (streaks, duels, forfeits) keeps people coming back for months. It is a habit tracker first; the game is seasoning.

## Brand Personality

**Tactile · Honest · Warm-competitive.** The UI is soft molded material you press — checking in feels like pushing a real button that stays pressed. Numbers never lie (strict streaks, soft-signal metadata instead of policing). Between friends it's warm trash-talk energy — Strava kudos, not corporate KPIs.
...
4. **Friends are the feature.** Shared surfaces (feed, duels, recap cards) get first-class craft — recap cards should beg to be screenshotted, like Strava.
